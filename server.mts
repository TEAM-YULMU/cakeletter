...
import { uploadBase64ImageToS3 } from './src/lib/s3.js';
...
app.prepare().then(() => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("onSend", async ({ memberId, roomId, chat, imageData }) => {
      console.log(memberId, roomId, chat);

      let url = "";

      if (imageData) {
        url = await uploadBase64ImageToS3({
          path: 'chat',
          imageData: imageData
        })
      }
